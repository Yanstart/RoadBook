...
export const calculateRoadbookStatistics = async (roadbookId: string) => {
  try {
    ...
    const roadTypesSet = new Set<string>();
    const roadTypesCounts: Record<string, number> = {};
    ...
    for (const session of sortedSessions) {
      if (lastSessionDate) {
        const daysDiff = Math.floor(
          (new Date(session.date).getTime() - new Date(lastSessionDate).getTime()) / 
          (1000 * 60 * 60 * 24)
        );
    ...
    const daysSinceCreation = Math.floor(
      (new Date().getTime() - new Date(roadbook.createdAt).getTime()) / 
      (1000 * 60 * 60 * 24)
    );
 ...
 */
export const getRoadbookById = async (id: string, userId: string, includeStats = false) => {
  try {
    ...
    if (includeStats) {
      const statistics = await calculateRoadbookStatistics(id);
      
      // Calculer les métriques de validation des sessions
      const validatedSessionsCount = roadbook.sessions.filter(
        session => session.validatorId !== null && session.validationDate !== null
      ).length;
...
const isValidStatusTransition = (
  ...
  const allowedTransitions = roadbookStateMachine[currentStatus as keyof typeof roadbookStateMachine] || [];
 ...
 */
const validateStatusTransitionConditions = async (
  id: string,
  currentStatus: string,
  newStatus: string
 ...
 */
const calculateTotalDuration = async (roadbookId: string): Promise<number> => {
  const sessions = await prisma.session.findMany({
    where: { roadbookId },
...
export const updateRoadbookStatus = async (id: string, status: string, userId: string) => {
  try {
    ...
    const stats = {
      totalSessions: await prisma.session.count({ where: { roadbookId: id } }),
 ...
 */
export const prepareRoadbookExportData = async (id: string, userId: string) => {
  try {
